...
import { RegistrationFlow, UpdateRegistrationFlowBody } from "../packages/kratos-cdk/index.ts"
...
const AuthRegisterForm = () => {
          ...
          for (const item of data.continue_with) {
            switch (item.action) {
              case "show_verification_ui":
                window.location.href = "/auth/verification?flow=" + item.flow.id
                return
            }
          }
        }
  ...
  return (
    <div className="bg-[#28282B] rounded-3xl px-6 py-8 flex flex-col space-y-5">
      ...
      <Flow flow={flow} onSubmit={onRegister} loading={flowLoading} />
