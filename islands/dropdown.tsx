...
const Dropdown = ({ children, items, width }: DropdownProps) => {
  const buttonRef = useRef<HTMLButtonElement>(null)
  const [state, setState] = useState(false)
  return (
    <div>
      <button ref={buttonRef} className="flex space-x-2 items-center" onClick={() => setState(!state)}>
        {children}
        <svg
          width="12"
          height="6"
      ...
      {state && (
        <div
          ...
          style={{
            ...
            right: `calc(100% - ${buttonRef?.current?.getBoundingClientRect().left}px - ${
              buttonRef?.current?.offsetWidth
            }px)`,
            minWidth: width || buttonRef?.current?.offsetWidth,
